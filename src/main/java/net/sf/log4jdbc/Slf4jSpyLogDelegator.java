...
public class Slf4jSpyLogDelegator implements SpyLogDelegator
{
    ...
    String sql, boolean debugInfo)
  {
    ...
    /*记录关键表的增删改beigin add by zhengwei 20130906*/
    Pattern insert_table =Pattern.compile("insert\\s*into\\s*(.*?)\\s");
    Matcher m_insert_table = insert_table.matcher(outString);
    Pattern update_table =Pattern.compile("update\\s*(.*?)\\s*set");
    Matcher m_update_table = update_table.matcher(outString);
    Pattern delete_table =Pattern.compile("delete\\s*from\\s*(.*?)\\s");
    Matcher m_delete_table = delete_table.matcher(outString);
    String table_name = null;
    DbOpType op_type = DbOpType.DEFAULT;
    if(m_insert_table.find()){
    	table_name = m_insert_table.group(1);
    	op_type = DbOpType.INSERT;
    }
    if(m_delete_table.find()){
    	table_name = m_delete_table.group(1);
    	op_type = DbOpType.DELETE;
    }
    if(m_update_table.find()){
    	table_name = m_update_table.group(1);
    	op_type = DbOpType.UPDATE;;
    }
    /*判断是否关键表*/
    if(AppProperties.isKeyTable(table_name)){
    	LogBusiUtils.log(outString, op_type);
    }
    /*记录关键表的增删改end add by zhengwei 20130906*/
    return outString;
  }
//########add by zhengwei ##########
//private void doBusiLog(String content, Integer op_type) {
//	String op_user="";
//	String ip="";
//	String subModule ="";
...
//		//子模块
//		subModule = ConvertUtils.convert(MDC.get("share_current_action_class_name"));
//		//地震序列
//		eq_id = ConvertUtils.convert(MDC.get("eq.id"));
//	} catch (Exception e) {
//		sqlTimingLogger.info("出现异常"+e);
//	}
//	
//	String op_date_string = DateConvertUtils.format(op_date, BaseEntity.DATE_TIME_FORMAT);
//	DbUtil.update("insert into log_busi values('"+StringUtils.remove(UUID.randomUUID().toString(), "-")+"',"+
//			op_type+",'"+op_user+"',to_date('"+op_date_string+"','"+"yy-MM-dd hh24:mi:ss"+"'),'"+subModule+"','"+ip+"','"+eq_id+"','"+content.replace("'", "\"")+"')");
//}
