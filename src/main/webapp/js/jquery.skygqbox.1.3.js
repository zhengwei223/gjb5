 ...
 * 添加一个延时自动关闭弹出层的功能
 * 添加了弹出层的显示位置的个性选择
 *插件官方地址：http://www.skygq.com/2011/01/05/jquery-simple-popup-overlay-1-3/
 */
;
(function($) {
	// 给页面装载CSS样式
	...
	$.skygqbox = function(elements, s) {
		...
		// 弹框的显示初始化
		var WRAP = '<div id="skygqOverlay"></div><div class="wrap_out" id="wrapOut"><div class="wrap_in" id="wrapIn"><div id="wrapBar" class="wrap_bar"  onselectstart="return false;"><div class="wrap_title"><span>'
				+ s.title
				+ '</span></div><a href="javascript:void(0);" class="wrap_close" id="wrapClose">'
				+ s.shut
				+ '</a></div><div class="wrap_body" id="wrapBody"></div></div></div>';
		$("body").append(WRAP);
	...
	$.skygqbox.getSize = function(o) {
		// 获取任意元素的高宽
		var w_h = {};
		$('<div id="wrapClone" style="position:absolute;left:-6000px;"></div>')
				.appendTo("body").append(o.clone());
	...
	$.skygqbox.setPosition = function() {
		...
		var w = $(window).width(), h = $(window).height(), ph = $("body")
				.height();
		if (ph < h) {
			ph = h;
		}
