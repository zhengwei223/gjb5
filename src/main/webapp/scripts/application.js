...
function batchOp(action, checkboxName, form) {
	if (!hasOneChecked(checkboxName)) {
		alert('请选择要操作的对象!');
		return;
	}
...
// ------add by zhengwei
$(function() {
	$(":button").addClass("stdButton");
	$(":submit").addClass("stdButton");
	$(":reset").addClass("stdButton");
	/*$("table.formTable tr").each(function(i) {
		$(this).addClass("mouseOut" + i % 2);
	});*/
});